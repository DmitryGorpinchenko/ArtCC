...
public:
    ...
    Items GetItems() const {
        ...
        for (size_t i = 0; i < counts.size(); ++i) {
            res[i] = { Airport(i), counts[i] };
        }
        return res;
    }

private:
    static size_t Id(TAirport airport) {
        return static_cast<size_t>(airport);
    }
    static TAirport Airport(size_t id) {
        return static_cast<TAirport>(id);
    }
