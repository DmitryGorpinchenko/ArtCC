...
array<double, VERTEX_COUNT> tree_add, tree_sub, tree_factor;
...
array<Record, VERTEX_COUNT> tree_values;
...
void Push(size_t v, size_t l, size_t r) {
    for (size_t w = v * 2; w <= v * 2 + 1; ++w) {
        if (w < VERTEX_COUNT) {
            ...
            (tree_values[w].earned *= tree_factor[v]) += tree_add[v] * (r - l) / 2;
...
Record ComputeSum(size_t v, size_t l, size_t r, size_t ql, size_t qr) {
...
void Add(size_t v, size_t l, size_t r, size_t ql, size_t qr, double value) {
    ...
    tree_values[v] =
        (v * 2 < VERTEX_COUNT ? tree_values[v * 2] : Record{0, 0})
      + (v * 2 + 1 < VERTEX_COUNT ? tree_values[v * 2 + 1] : Record{0, 0});
}
...
int main() {
    ...
    for (int i = 0; i < q; ++i) {
        ...
        if (query_type == "ComputeIncome") {
            cout << ComputeSum(1, 0, DAY_COUNT_P2, idx_from, idx_to).Income() << endl;
        } else if (query_type == "PayTax") {
            int percent;
            cin >> percent;
            Multiply(1, 0, DAY_COUNT_P2, idx_from, idx_to, 1. - percent / 100.);
