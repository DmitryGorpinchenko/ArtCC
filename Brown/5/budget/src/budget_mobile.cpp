...
struct Record {
    ...
    double Income() const { return earned - spent; }
};
...
public:
    ...
    Record Collapse(Record origin, IndexSegment segment) const {
        return {
            origin.earned * tax_.factor + add_.delta * segment.length(),
            origin.spent + subtract_.delta * segment.length(),
        };
    }
...
class BudgetManager : public SummingSegmentTree<Record, BulkLinearUpdater> {
public:
...
struct ComputeIncomeRequest : ReadRequest<double> {
    ...
    double Process(const BudgetManager& manager) const override {
        return manager.ComputeSum(MakeDateSegment(date_from, date_to)).Income();
    }
...
struct SpendRequest : ModifyRequest {
    ...
    void ParseFrom(string_view input) override {
        date_from = Date::FromString(ReadToken(input));
        date_to = Date::FromString(ReadToken(input));
        amount = ConvertToInt(input);
    }

    void Process(BudgetManager& manager) const override {
        const auto date_segment = MakeDateSegment(date_from, date_to);
        const double daily_amount = amount * 1.0 / date_segment.length();
        manager.AddBulkOperation(date_segment, BulkMoneySubtractor{daily_amount});
    }
...
struct PayTaxRequest : ModifyRequest {
    ...
    void Process(BudgetManager& manager) const override {
        manager.AddBulkOperation(MakeDateSegment(date_from, date_to), BulkTaxApplier{1. - percent / 100.});
    }
