...
void TestComputeMedianAge() {
    ...
    {
        vector<Person> p = { {23, Gender::MALE, true}, {123, Gender::MALE, true}, {56, Gender::MALE, true} };
        ASSERT_EQUAL(ComputeMedianAge(begin(p), end(p)), 56);
        
        ASSERT_EQUAL(ComputeStats(p).females, 0);
    }
}

void TestReadPersons() {
    istringstream ss("1\n23 0 1");
    const auto res = ReadPersons(ss);
    vector<Person> expected = { { 23, Gender::FEMALE, true } };
    ASSERT_EQUAL(res, expected);
}

void TestPrintStats() {
    AgeStats stats = { 10, 9, 1, 9, 0, 0, 1 };
