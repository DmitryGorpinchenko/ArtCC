...
private:
    ...
    static vector<const Domain*> AbsorbSubdomains(vector<const Domain*> domains) {
        domains.erase(
            unique(begin(domains), end(domains), [](const Domain* lhs, const Domain* rhs) {
...
void TestSplit() {
    const auto parts = Split("m.ya.ru", ".");
    vector<string_view> expected = { "m", "ya", "ru" };
    ASSERT_EQUAL(parts, expected);
}

void TestReversedParts() {
    Domain d("m.ya.ru");
    const auto range = d.GetReversedParts();
    vector<string> parts(begin(range), end(range));
    vector<string> expected = { "ru", "ya", "m" };
    ASSERT_EQUAL(parts, expected);
}

void TestIsSubdomain() {
    ASSERT(IsSubdomain(Domain("m.ya.ru"), Domain("m.ya.ru")));
    ASSERT(IsSubdomain(Domain("m.ya.ru"), Domain("ya.ru")));
    ASSERT(!IsSubdomain(Domain("ya.ru"), Domain("m.ya.ru")));
}

void TestCheckDomains() {
    const auto res = CheckDomains({ Domain("ya.ru"), Domain("m.ya.ru"), Domain("mail.ru") },
                                  { Domain("ru"), Domain("com"), Domain("ya.ru"), Domain("a.mail.ru"), Domain("n.ya.ru") });
...
void TestReadDomains() {
    istringstream ss("2\nm.ya.ru\nmail.ru");
    const auto res = ReadDomains(ss);
    vector<Domain> expected = { Domain("m.ya.ru"), Domain("mail.ru") };
    ASSERT_EQUAL(res, expected);
}
