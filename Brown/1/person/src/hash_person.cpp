...
template <typename CoefIt>
size_t Poly(CoefIt first, CoefIt last, size_t x) {
    size_t res = 0;
    while (last != first) {
        res = res * x + *--last;
    }
...
struct AddressHasher {
    size_t operator ()(const Address& a) const {
        const hash<string> hash_str;
        const hash<int> hash_int;
        const auto coefs = { hash_str(a.city),
                             hash_str(a.street),
                             hash_int(a.building) };
        return Poly(begin(coefs), end(coefs), 37);
    }
};
...
struct PersonHasher {
    size_t operator ()(const Person& p) const {
        const hash<string> hash_str;
        const hash<int> hash_int;
        const hash<double> hash_double;
        const AddressHasher hash_address;
        const auto coefs = { hash_str(p.name),
                             hash_int(p.height),
                             hash_double(p.weight),
                             hash_address(p.address) };
        return Poly(begin(coefs), end(coefs), 37);
    }
};
