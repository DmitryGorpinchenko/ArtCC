...
class Polynomial {
    ...
    class CoeffProxy {
        ...
        size_t deg;
    public:
        CoeffProxy(Polynomial& poly, size_t d)
            : owner(poly)
            , deg(d)
        {}
    
        operator T() const {
            return static_cast<const Polynomial&>(owner)[deg];
        }
    
        T operator =(T val) {
            owner.SetCoeff(deg, val);
            return val;
        }
    };
    ...
    void SetCoeff(size_t degree, T val) {
        if (Degree() < degree) {
...
public:
    ...
    CoeffProxy operator [](size_t degree) {
        return CoeffProxy(*this, degree);
    }
...
void TestNonconstAccess() {
    ...
    poly[0] = poly[5] = poly[9] = 223;
    ASSERT_EQUAL(poly[0], 223);
    ASSERT_EQUAL(poly[5], 223);
    ASSERT_EQUAL(poly[9], 223);

    {
        LOG_DURATION("Iteration over const");
        for (size_t i = 10; i < 50000; ++i) {
            ASSERT_EQUAL(std::as_const(poly)[i], 0);
            ASSERT_EQUAL(poly.Degree(), 9);
        }
    }
    {
