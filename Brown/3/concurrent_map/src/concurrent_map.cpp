...
public:
    ...
    public:
        ...
        explicit WriteAccess(Bucket& bucket, const K& key)
            : lock(bucket.m)
    ...
    public:
        ...
        explicit ReadAccess(const Bucket& bucket, const K& key)
            : lock(bucket.m)
    ...
    WriteAccess operator [](const K& key) {
        return WriteAccess(GetBucket(key), key);
    }
    ReadAccess At(const K& key) const {
        return ReadAccess(GetBucket(key), key);
    }

    bool Has(const K& key) const {
        const auto& b = GetBucket(key);
        shared_lock lock(b.m);
        return b.data.count(key);
    }

    MapType BuildOrdinaryMap() const {
        MapType res;
        for (auto& [m, data] : buckets) {
            shared_lock lock(m);
            res.insert(begin(data), end(data));
        }
        return res;
    }
...
private:
    ...
    const Bucket& GetBucket(const K& key) const { return buckets[GetBucketId(key)]; }
    Bucket& GetBucket(const K& key) { return buckets[GetBucketId(key)]; }
